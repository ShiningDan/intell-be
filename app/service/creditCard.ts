...
export default class CreditCard extends Service {
   ...
   */
  public async getRecActAndCard( {act_num, card_num }: {act_num: number, card_num: number} ): Promise<[CardActivityModel[], CardInfoModel[]]> {
    const { app } = this
  ...
  public async fastMatch( {minMonthIncome, maxMonthIncome, amount, phone}: {minMonthIncome: number, maxMonthIncome: number,
    amount: number, phone: string} ): Promise<CardInfoModel | undefined> {
    const { app } = this
    const cardRepository  = app.typeorm.getRepository(CardInfoModel)
    // 这里先模拟一个返回
    if ((minMonthIncome > 0) && (maxMonthIncome < 100000) && (amount > 0) && (phone.length === 11)) {
      const card = await cardRepository.findOne({
        // 只需要返回以下字段
        select: ['id', 'name', 'img', 'bank', 'baseInfo', 'priInfo', 'feeInfo', 'condiInfo']
      })
      return card
    }else {
  ...
  public async apply( {card_id, name, sex, id_number, phone, income_min, income_max, amount, address}: {
    ...
    income_max: number, amount: number, address: string}): Promise<CardUserModel> {
    const { app } = this
    const userRepository  = app.typeorm.getRepository(CardUserModel)
    const user = new CardUserModel()
    user.cardId = card_id
    user.name = name
    user.sex = sex
    user.idNumber = id_number
    user.phone = phone
    user.incomeMin = income_min
    user.incomeMax = income_max
    user.amount = amount
  ...
  public async activity({ id }: {id: number}): Promise<object> {
    ...
    const act = await actRepository.findOne({
      ...
      // 只需要返回以下字段
      select: ['id', 'title', 'desc', 'time', 'area', 'fullMsg']
    })
    // 查询参与该活动的卡id
