...
export default class Ins extends Service {
   ...
   * @returns 保险数组
   */
  public async getRecommendIns({ limits }: {limits: number}): Promise<InsInfoModel[]> {
    const { app } = this
   ...
   * @returns:对应的保险对象内容
   */
  public async getInsContent({ id }: {id: number}): Promise<InsInfoModel> {
    const { app } = this
    const insRepository = app.typeorm.getRepository(InsInfoModel)
   ...
   * @returns :添加成功的对象
   */
  public async applyUser({ phone }: {phone: string}): Promise<InsUserModel> {
    const { app } = this
    const userRepository = app.typeorm.getRepository(InsUserModel)
    // 根据电话号创建匿名User
    const user = new InsUserModel()
    user.phone = phone
